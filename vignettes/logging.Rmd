...
__Caution__: When initializing the logging system, the folder where log files will be saved is not defined yet and left as `NULL`.
...
The function `logCatch()` can be used to mask specific messages when handling message conditions (i.e. error, warning, message).
...
It is possible to define specific calls that should be masked using the `setErrorMasking()`, `setWarningMasking()` and `setInfoMasking()` functiona.
